...
class FaceAnalyzer:
    def __init__(
        ...
        # Overlay state (updated by analysis thread, read by main thread)
    ...
    def run(self) -> FaceAnalysisResult:
        """
        Opens default camera, shows live window.
        Press ENTER (in terminal) or Q (in window) to stop.
        Returns FaceAnalysisResult with average score.
        """
        ...
        if not self._cap.isOpened():
            ...
            if not self._cap.isOpened():
                raise RuntimeError("Could not open default camera. Make sure no other app is using it.")
        ...
        print("[FaceAnalyzer] Loading DeepFace models (first run ~10s, please wait)...")
        ...
        print("\n[FaceAnalyzer] Running. Press ENTER or Q to stop and see results.\n")

        # ── Main thread: show camera window ──────
        while self._running:
            ...
            if key == ord('q') or key == 13:   # Q or Enter key in window
    ...
    def _wait_for_enter(self):
        input()   # blocks until user presses Enter in terminal
    ...
    def _analyze_frame(self, frame: np.ndarray, timestamp: float) -> Optional[EmotionSnapshot]:
        try:
            ...
            face_score = round(
                min(max(sum(EMOTION_SCORES.get(e, 0.5) * p for e, p in norm_emotions.items()), 0.0), 1.0), 4
            )
            ...
            snapshot_b64 = self._crop_and_encode(frame, region) if (self.save_face_crops and region) else None
...
def extract_fusion_payload(result: FaceAnalysisResult) -> dict:
    return {
        ...
        "face": {
            ...
            "timeline": [
                {"t": s.timestamp, "emotion": s.dominant_emotion,
                 "score": s.face_score, "confidence": s.confidence}
...
if __name__ == "__main__":
    ...
    analyzer = FaceAnalyzer(
        ...
        snapshot_interval=2.0,       # analyze every 2 seconds
        detector_backend="opencv",   # fast; change to "retinaface" for higher accuracy
    ...
    print("\n  Fusion Payload (for voice merge):")
